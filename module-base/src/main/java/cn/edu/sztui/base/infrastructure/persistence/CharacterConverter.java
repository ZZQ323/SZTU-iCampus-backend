package cn.edu.sztui.base.infrastructure.persistence;
...
public class CharacterConverter {
    ...
    public static String toDBC(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            // 半角转全角：加65248
            if (cCode >= 0x21 && cCode <= 0x7E) {
                cCode += 65248;
            }
            // 处理半角空格（0x0020）转全角空格（0x3000）
            else if (cCode == 0x0020) {
                cCode = 0x3000;
            }
    ...
    private static char toHalfWidthCharComplete(char c) {
        ...
        // 处理数字
        if (code >= 0xFF10 && code <= 0xFF19) { // 全角数字0-9
            return (char) (code - 0xFF10 + '0');
        }

        // 处理大写字母
        if (code >= 0xFF21 && code <= 0xFF3A) { // 全角A-Z
            return (char) (code - 0xFF21 + 'A');
        }

        // 处理小写字母
        if (code >= 0xFF41 && code <= 0xFF5A) { // 全角a-z
            return (char) (code - 0xFF41 + 'a');
        }
